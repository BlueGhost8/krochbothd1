...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
    msg.reply('**Aleyküm selam, Hoşgeldin İyisindir İşşallah =)**');
  }
});
...
client.on("guildMemberAdd", async member => {
  ...
  const skanal31 = member.guild.channels.find('name', skanal9)
    skanal31.send(`**:inbox_tray: ${member.user.tag}** Katıldı **${sayac}** kişi olmamıza son **${sayac - member.guild.members.size}** üye kaldı ✅`)
});
...
client.on("guildMemberRemove", async member => {
  ...
  if (!skanal9) return;
  const skanal31 = member.guild.channels.find('name', skanal9)
    skanal31.send(`**:outbox_tray: ${member.user.tag}** Ayrıldı **${sayac}** kişi olmamıza son **${sayac - member.guild.members.size}** üye kaldı ❌`)
});
